 ...
 */
@Component("hBaseBeanDaoUtil")
public class HBaseBeanDaoUtil {

    protected final org.slf4j.Logger logger = LoggerFactory.getLogger(this.getClass());

    // 关闭连接
    public static void close() {
        if (HBaseConnectionFactory.connection != null) {
            try {
                HBaseConnectionFactory.connection.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public <T> List<T> queryScan(T obj, Map<String, String> param)throws Exception{
        ...
        try (Table table = HBaseConnectionFactory.connection.getTable(TableName.valueOf(tableName)); Admin admin = HBaseConnectionFactory.connection.getAdmin();){
    ...
    private void delete(List<Delete> deletes, String tableName) {
        try (Table table = HBaseConnectionFactory.connection.getTable(TableName.valueOf(tableName));) {
            if (StringUtils.isBlank(tableName)) {
